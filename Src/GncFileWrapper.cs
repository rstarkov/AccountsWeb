...
public class GncFileWrapper
{
    ...
    /// <summary>If not null, every web request will respond with this error message.</summary>
    [ClassifyIgnore]
    public HtmlTag GlobalErrorMessage = null;

    /// <summary>
    ///     Holds the currently opened GnuCash Session. "null" if the session hasn't been loaded (eg no GnuCashFile is
    ///     configured).</summary>
    [ClassifyIgnore]
    public GncSession Session;

    /// <summary>Holds the main book from the currently opened GnuCash <see cref="Session"/>.</summary>
    [ClassifyIgnore]
    public GncBook Book;

    /// <summary>
    ///     Holds the name of the file this class was loaded from. "null" if this is a new file that has never been saved.</summary>
    [ClassifyIgnore]
    ...
    /// <summary>
    ///     Reloads the <see cref="GncSession"/> underlying the current open file.</summary>
    /// <remarks>
    ///     <para>
    ///         Throws an exception if no file is open.</para>
    ///     <para>
    ///         Does not throw any exceptions on any other error - instead configures a <see cref="GlobalErrorMessage"/> to
    ///         explain the problem.</para></remarks>
    ...
    /// <summary>
    ///     Reloads the <see cref="GncSession"/> underlying the current open file only if the gnucash file has changed.
