...
namespace AccountsWeb
{
    public class PageReconcile : WebPage
    {
        ...
        public override object GetContent()
        {
            ...
            if (btnPressed != btn.None)
            {
                ...
                if (btnPressed == btn.ReconcilePreset || btnPressed == btn.ReconcileCustom)
                {
                    ...
                    var accountSplits = account.EnumSplits(false).Where(spl => !spl.IsBalsnap && spl.Transaction.DatePosted >= fromDate && spl.Transaction.DatePosted <= toDate).ToList();
        ...
        {
            bool haveDr = matchDr.Success && matchDr.Value.Trim() != "-";
            bool haveCr = matchCr.Success && matchCr.Value.Trim() != "-";
            if (haveDr == haveCr)
